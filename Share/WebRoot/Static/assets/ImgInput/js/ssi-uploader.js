...
(function (root, factory) {
    ...
    Ssi_upload.prototype.init = function (element) {
        ...
        /*
         ...
         '<button class="ssi-button success">' + '选择图片' + '</button>' +
         '</span>').append(element);
         */


        var $chooseBtn = $('<span class="ssi-InputLabel">' +
            '<a href="javascript:void(0)" class="sendarea_a light_gray" id="btn-image">' +
        ...
        //将find中的button换成我们自己的按钮
        $chooseBtn.find('#btn-image').click(function () {
            $input.trigger('click');
        });
        ...
        $uploadBox.on('click', '.ssi-removeBtn', function (e) { //remove the file from list
            ...
            if (thisS.currentListLength === 0) { // if no more files in the list
                ...
                $('#ssi-previewBox').css('visibility', 'hidden'); //修改 2: 如果删除图片是最后一张, 面板隐藏
            }
        });
...
//----------------------------UPLOADFILES------------------------------------
    ...
    Ssi_upload.prototype.toUploadFiles = function (files) {
        ...
        function setupReader() {
            $('#ssi-previewBox').css('visibility', 'visible'); //修改 1: 选中图片, 才会显示图片面板

            //当输入框没文字时，自动追加四个字
            if ($('#share_content').val().length == 0) {
                $('#share_content').val('分享图片');
                $('.words_number').text('296');
            }
