...
function getProfile(userId, containerId) {
    $.ajax({
        ...
        success: function (data) {
            ...
            html += '<a href="' + path + '/index/personalPage.do?userId=' + user.id + '">';
            ...
            html += '<div class="profile_username">',
                html += '<a href="' + path + '/index/personalPage.do?userId=' + user.id + '">' + user.username + '</a>';
            if (user.status == 1) {
                html += '<span class="pinbi">(屏蔽中)</span>';
            }
            html += '</div>';
            html += '<ul class="profile_info">';
            html += '<li> <a href="' + path + '/index/followingPage.do?userId=' + userId + '"><strong id="followCount">' + user.followCount + '</strong><span>关注</span></a> </li>';
            html += '<li> <a href="' + path + '/index/fansPage.do?userId=' + userId + '"><strong id="fansCount">' + user.fansCount + '</strong><span>粉丝</span></a> </li>';
            html += '<li> <a href="' + path + '/index/personalPage.do?userId=' + user.id + '"><strong>' + user.shareCount + '</strong><span>分享</span></a> </li>';
            html += '</ul>';
            html += '</div>';
            $container.html(html);
        ...
        success: function (data) {
            ...
            if (type == 1) {
                ...
                for (var i = 0; i < fansList.length; i++) {
                    html += '<li>';
                    html += '<a href="' + path + '/index/personalPage.do?userId=' + fansList[i].id + '" style="display:block"><img src="' + path + '/' + fansList[i].userImg + '" class="following_avatar"></a>';
                    html += '<a href="' + path + '/index/personalPage.do?userId=' + fansList[i].id + '"><span>' + fansList[i].username + '</span></a>';
                    html += '</li>';
                }
                html += '</ul>';
                html += '</div>';
                html += '<div class="follower_more">— <a href="' + path + '/index/fansPage.do?userId=' + userId + '">查看更多</a> —</div>';
                $container.html(html);
            } else if (type == 2) {
                ...
                for (var i = 0; i < fansList.length; i++) {
                    ...
                    html += '<div class="follow_container follow_card">' +
                        ...
                        '        <a href="' + path + '/index/personalPage.do?userId=' + fansList[i].id + '"><img' +
                        '               src="' + path + '/' + fansList[i].userImg + '" class="thumbnail"></a>' +
                        '   </div>' +
                        '    <div class="follow_info_container">' +
                        '        <div class="follow_user">' +
                        '            <a href="' + path + '/index/personalPage.do?userId=' + fansList[i].id + '"' +
                        ...
                        '            <ul>' +
                        '                <li>' +
                        '                    <a href="' + path + '/index/followingPage.do?userId=' + fansList[i].id + '"><span>关注</span><span>' + fansList[i].followCount + '</span></a>' +
                        '                </li>' +
                        '                <li>' +
                        '                    <a href="' + path + '/index/fansPage.do?userId=' + fansList[i].id + '"><span>粉丝</span><span>' + fansList[i].fansCount + '</span></a>' +
                        '                </li>' +
                        '                <li>' +
                        '                    <a href="' + path + '/index/personalPage.do?userId=' + fansList[i].id + '">' +
                        '                        <span>分享</span><span>' + fansList[i].shareCount + '</span>' +
                        '                    </a>' +
                        '                </li>' +
                        '            </ul>' +
                        '        </div>' +
        ...
        success: function (data) {
            ...
            if (type == 1) {
                ...
                for (var i = 0; i < followingList.length; i++) {
                    html += '<li>';
                    html += '<a href="' + path + '/index/personalPage.do?userId=' + followingList[i].id + '" style="display:block"><img src="' + path + '/' + followingList[i].userImg + '" class="following_avatar"></a>';
                    html += '<a href="' + path + '/index/personalPage.do?userId=' + followingList[i].id + '"><span>' + followingList[i].username + '</span></a>';
                    html += '</li>';
                }
                html += '</ul>';
                html += '</div>';
                html += '<div class="following_more">— <a href="' + path + '/index/followingPage.do?userId=' + userId + '">查看更多</a> —</div>';
                $container.html(html);
            } else if (type == 2) {
                //初始化头部的显示
                $("#username").html(user.username);
                ...
                for (var i = 0; i < followingList.length; i++) {
                    ...
                    html += '<div class="follow_container follow_card">' +
                        '   <div class="follow_avatar">' +
                        '        <a href="' + path + '/index/personalPage.do?userId=' + followingList[i].id + '"><img' +
                        '               src="' + path + '/' + followingList[i].userImg + '" class="thumbnail"></a>' +
                        '   </div>' +
                        '    <div class="follow_info_container">' +
                        '        <div class="follow_user">' +
                        '            <a href="' + path + '/index/personalPage.do?userId=' + followingList[i].id + '"' +
                        ...
                        '            <ul>' +
                        '                <li>' +
                        '                    <a href="' + path + '/index/followingPage.do?userId=' + followingList[i].id + '"><span>关注</span><span>' + followingList[i].followCount + '</span></a>' +
                        '                </li>' +
                        '                <li>' +
                        '                    <a href="' + path + '/index/fansPage.do?userId=' + followingList[i].id + '"><span>粉丝</span><span>' + followingList[i].fansCount + '</span></a>' +
                        '                </li>' +
                        '                <li>' +
                        '                    <a href="' + path + '/index/personalPage.do?userId=' + followingList[i].id + '">' +
                        '                        <span>分享</span><span>' + followingList[i].shareCount + '</span>' +
                        '                    </a>' +
                        '                </li>' +
                        '            </ul>' +
                        '        </div>' +
 ...
 */
function judgeUrl() {
    if (window.location.href.indexOf("fansPage") > 0) {
        $("#hisFans").css({
            'border-bottom-color': '#39896b',
            'border-bottom-width': '3px',
            'border-bottom-style': 'solid'
        });
    } else if (window.location.href.indexOf("followingPage") > 0) {
        $("#hisFollow").css({
            'border-bottom-color': '#39896b',
            'border-bottom-width': '3px',
            'border-bottom-style': 'solid'
        ...
        success: function (data) {
            ...
            $.each(userList, function (index, user) {
                ...
                html += '	<div class="result_user">' +
                    ...
                    '            <a href="' + path + '/index/personalPage.do?userId=' + user.id + '">' +
