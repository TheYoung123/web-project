...
(function ($) {
    ...
    jQuery.fn.actizPicShow = function (option) {
        ...
        option.oneWidth = option.oneWidth || 200;//单个图片显示宽度
        option.oneHeight = option.oneHeight || 200;//单个图片显示高度
        //修改
        ...
        function narrowMorePicShow() {//多图片 小图片显示
            ...
            for (var i = 0; i < option.data.length; i++) {
                ...
                morepicArray.push('<img id="' + option.data[i].id + '" height="100%" width="100%" class="bigcursor" curLoc="' + i + '" ');
        ...
        me.delegate("#expandCenterImg", "mousemove", function (e) {
            ...
            } else if (switchWidth <= 528 && switchWidth >= 378 && current_i != option.data.length - 1) {
                $(this).removeClass("leftcursor").removeClass("smallcursor").addClass("rightcursor");
            } else if (switchWidth <= 528) {
        ...
        me.delegate(".bigcursor", "click", function (e) {
            ...
            option.bigWidth = option.bigWidth || $('.actizPicShowExpand').innerWidth();
            option.bigHeight = option.bigHeight || $('.actizPicShowExpand').innerHeight();

            console.log('width: ' + option.bigWidth);
            console.log('height: ' + option.bigHeight);
        });
        //绑定图片收藏事件....
